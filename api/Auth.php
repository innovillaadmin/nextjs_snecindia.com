...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($data->action) && $data->action == 'resetpassword') {
        ...
        if ($userid > 0 && $newpass != '') {
            $qry = $conn->query("update userdata set userpassword = '$newpass', firstpasschange='y' where id ='$userid'");
            if ($qry && ! $conn->error) {
                echo json_encode([
                    'status' => 'success',
                ]);
    ...
    if (isset($data->action) && $data->action === 'userRegistration') {
        ...
        if (empty($fname) || empty($contact) || empty($password)) {
            ...
            $checkuser = $conn->query("select id from userdata where usercontact = '$contact' or useremail ='$email'");
